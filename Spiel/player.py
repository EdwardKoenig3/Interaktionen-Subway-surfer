...
class Player(Entity):
    ...
    def __init__(self):
        super().__init__(
            model='cube', color=C_PLAYER,
            scale=(0.7, PLAYER_SY, 0.7),
            position=(0, PLAYER_BASE_Y, 0),
        )
        self.head = Entity(parent=self, model='sphere', color=C_SKIN,
                           scale=(0.7, 0.7, 0.7), position=(0, 0.58, 0))
        self.pack = Entity(parent=self, model='cube', color=C_PACK,
    ...
    def update(self):
        ...
        if self.is_jumping:
            ...
            if self.vel_y <= 0 and self.y <= self._base_y:
                ...
                self.vel_y      = 0.0
                self.is_jumping = False
                self.platform   = None      # Plattform aufheben beim Bodenkontakt
